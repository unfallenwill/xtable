        ...
        match (method.as_str(), key.is_empty()) {
            ...
            ("POST", true) => {
                ...
                let xml = format!(
                    ...
                    deleted
                        .iter()
                        .map(|k| format!("<Deleted><Key>{}</Key></Deleted>", k))
                        .collect::<Vec<_>>()
                        .join("")
                );
...
async fn e2e_coordinator_commit_writes_object_to_backend() {
    ...
    assert!(
        !mock.snapshot_keys().contains(&"k".to_string()),
...
async fn e2e_atomic_multi_object_all_or_nothing() {
    ...
    assert!(
        !snap.contains(&"a".to_string()),
        "spec §5.1 removed per-record PUT; backend must be empty: saw {:?}",
        snap
    );
...
async fn e2e_mvcc_reader_at_old_snapshot_p_sees_old_value() {
    ...
    for (i, sz) in sizes.iter().enumerate() {
        ...
        let body = vec![b'x'; *sz];
        coord
            .stage(&txn, &key, body, None, HashMap::new(), false)
            .await
            .unwrap();
...
async fn e2e_mvcc_two_readers_different_snapshots_see_different_states() {
    ...
    let t1 = coord.begin(None).await.unwrap();
    coord
        .stage(&t1, &key, b"v1".to_vec(), None, HashMap::new(), false)
        .await
        .unwrap();
    ...
    let got_at_v1 = store
        .read_at_snapshot("k", r1.commit_version)
        .unwrap()
        .unwrap();
    ...
    let t2 = coord.begin(None).await.unwrap();
    coord
        .stage(&t2, &key, b"v2".to_vec(), None, HashMap::new(), false)
        .await
        .unwrap();
    ...
    let got_at_v2 = store
        .read_at_snapshot("k", r2.commit_version)
        .unwrap()
        .unwrap();
...
async fn e2e_mvcc_ssi_conflict_one_winner() {
    ...
    let store =
        LocalStore::open_path(&tempfile::TempDir::new().unwrap().path().join("xt.redb")).unwrap();
    // Seed chain with v=5.
    store
        .append_chain_entry(
            "k",
            &VersionEntry::new(5, "e5".into(), "k".into(), "init".into(), 10),
        )
        .unwrap();
    ...
    store
        .append_chain_entry(
            "k",
            &VersionEntry::new(6, "e6".into(), "k".into(), "A".into(), 1),
        )
        .unwrap();
    ...
    let chain_before = {
        ...
        for i in 1..=5u64 {
            store
                .append_chain_entry(
                    "k",
                    &VersionEntry::new(i, format!("e{}", i), "k".into(), format!("T{}", i), 10),
                )
                .unwrap();
        }
...
async fn e2e_put_object_large_body_uses_multipart() {
    ...
    assert!(
        result.etag.contains("multipart-etag-"),
        "got etag={}",
        result.etag
    );
